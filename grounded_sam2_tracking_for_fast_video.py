    ...
    def predict(
        ...
        if self.use_sam2_tracker:
            if self.frame_id == 0:
                ...
                img_bgr, track_detections = self.perceptron_video(img_rgb)
                if not self.frame_id % self.detection_step:
                    img_bgr, detections = self.perceptron_img(img_rgb)
                    objects_count, new_detections = update_new_masks(detections, track_detections, iou_threshold=0.6, objects_count=self.objects_count)
